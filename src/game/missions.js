const POOL = [
  { type: "satisfy_customers", target: (level) => Math.min(5, 2 + Math.floor(level / 3)), description: (n) => `お客さまを${n}人満足させよう`, reward: { points: 100 } },
  { type: "reach_sales", target: (level) => 5000 + level * 3000, description: (n) => `売上を${n.toLocaleString()}コイン達成しよう`, reward: { money: 500 } },
  { type: "special_orders", target: (level) => Math.min(3, Math.max(1, Math.floor(level / 3))), description: (n) => `スペシャルオーダーを${n}回達成しよう`, reward: { points: 200, materials: { strawberry: 2 } } },
