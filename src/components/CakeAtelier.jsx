...
export default function CakeAtelier({ state, onBuy, onEquip, onReset }) {
  ...
  return <section className="cakeAtelier card">
    ...
    <div className="cakePreview"><CakeModel style={style} recipe={recipe}/><span role="status">{part ? `${part.name}を試着中` : "いまのおめかし"}</span></div>
    {part && <button className="linkBtn" onClick={()=>setPreview(null)}>試着をやめる</button>}
    <div className="atelierTools">
      <button className="linkBtn" disabled={!state.cakeStyle?.band} onClick={() => { onReset("band"); setPreview(null); }}>帯を外す</button>
      <button className="linkBtn" disabled={state.cakeStyle?.top === "berry" && !state.cakeStyle?.band} onClick={() => { onReset("all"); setPreview(null); }}>定番のおめかしにもどす</button>
    </div>
    <p className="atelierInventoryNote">購入した飾りは、外しても手元に残ります。</p>
    <div className="partsGrid">{CAKE_PARTS.map(p=> {
      ...
      return <article key={p.id} className={`partCard ${preview === p.id ? "selected" : ""}`}>
        <button className="partPreview" onClick={()=>setPreview(p.id)} aria-pressed={preview === p.id} aria-label={`${p.name}を試着`}><div className="partThumbnail" aria-hidden="true"><CakeModel recipe={recipe} style={{...state.cakeStyle,[p.category]:p.id}}/></div><strong>{p.name}</strong></button>
        <p>{p.note}</p>
