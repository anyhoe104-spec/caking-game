...
export default function CakeModel({ style = {}, className = "", recipe = "ショートケーキ" }) {
  ...
  return <svg viewBox="0 0 240 200" className={`cakeModel ${className}`} role="img" aria-label={`${recipe}のおめかしプレビュー`} data-shape={shape}>
    <defs>
      <linearGradient id={`${id}-sponge`}><stop stopColor="#d6ab70"/><stop offset=".38" stopColor="#f7dfb0"/><stop offset="1" stopColor="#cba06c"/></linearGradient>
      <linearGradient id={`${id}-cream`} x2="0" y2="1"><stop stopColor="#fffdf4"/><stop offset="1" stopColor="#e9d4ad"/></linearGradient>
      <radialGradient id={`${id}-plate`}><stop offset=".7" stopColor="#fffaf0"/><stop offset=".86" stopColor="#dfccb0"/><stop offset="1" stopColor="#fff6e2"/></radialGradient>
    </defs>
    <ellipse cx="120" cy="171" rx="95" ry="17" fill="#76503e" opacity=".14"/>
    <ellipse cx="120" cy="160" rx="98" ry="22" fill={`url(#${id}-plate)`} stroke="#c9af86" strokeWidth="2"/>
    {shape === "pudding" ? <>
    ...
    <path d="M42 82v65c0 31 156 31 156 0V82" fill={`url(#${id}-sponge)`} stroke="#ba946b" strokeWidth="2"/>
    <path d="M43 113q77 34 154 0v13q-77 34-154 0" fill="#c56a75"/>
    <path d="M43 125q77 34 154 0v9q-77 34-154 0" fill="#fff5df"/>
    <ellipse cx="120" cy="82" rx="78" ry="25" fill={`url(#${id}-cream)`} stroke="#d9b995" strokeWidth="2"/>
    <path d="M42 83q10 12 14 20 7 12 13-2 5-9 13 8 6 14 13 0 6-10 15 5 7 11 14-2 6-11 14-1 7 9 13-7 6-10 15-2 8 5 13-8 8-8 19-11" fill={`url(#${id}-cream)`}/>
