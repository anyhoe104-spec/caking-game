...
await p.getByText('演出をスキップ',{exact:true}).evaluateAll(buttons=>buttons[0]?.click());await p.waitForTimeout(1600);assert.equal(await p.getByText('演出をスキップ',{exact:true}).count(),0);await p.getByText('工房にもどる',{exact:true}).click();
 ...
 await p.screenshot({path:path.join(QA,'craft.png')});await p.getByText('工房にもどる',{exact:true}).click();await p.getByRole('button',{name:'次の日へ'}).click();assert.equal((await save()).dayNumber,2);await p.clock.resume();
 ...
 await p.getByRole('button',{name:'レシピ',exact:true}).click();await p.getByRole('button',{name:'つくる',exact:true}).first().click();await p.getByText('演出をスキップ',{exact:true}).evaluateAll(buttons=>buttons[0]?.click());await p.getByText('工房にもどる',{exact:true}).click();await p.getByText('つづける',{exact:true}).click();
 await p.getByRole('button',{name:'レシピ',exact:true}).click();await p.getByRole('button',{name:'つくる',exact:true}).first().click();assert.equal(await p.locator('.productionCard').count(),1);await p.getByText('演出をスキップ',{exact:true}).evaluateAll(buttons=>buttons[0]?.click());await p.getByText('工房にもどる',{exact:true}).click();
 ...
 for(const recipe of RECIPES){
   ...
   await p.clock.runFor(950);assert.equal(await p.locator('.craftStation--'+profile.steps[1].kind).count(),1);
   await p.clock.runFor(800);assert.equal(await p.locator('.craftStation--'+profile.steps[2].kind).count(),1);
